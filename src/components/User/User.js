...
editClickHandler = (id) => {
  this.setState({
    ...
    render() {
      ...
      return (
          ...
          <div className="editedHoaInfo">
                ...
                <input
                    ...
                    name="mailingaddress"
                />
                <button className="saveChangeButton" onClick={ () => this.onEditSubmitHandler(curr.id)}>Save Changes</button>
          </div> :
          <div>
            <div className="hoaDivInfo">
            ...
            <button className="editHoaButton" onClick={ () => this.editClickHandler(curr.id)}>Edit HOA</button> : null}
            </div>}
          </div>
        )
      }) ;
      ...
      return (
        <div>
        <p className="HoaSearch">HOA Search</p>
              <form onSubmit={this.onSubmitHandler}>
                <input
                    ...
                    name="hoaname"
                />
                  <button className="selectHoaButton" onClick={ () => this.props.getHoaInfo(hoaname).then(() => this.props.getComments(this.props.hoa.searchResult[0].id))}>Select HOA</button>
              </form>
          {searchParam}
          {(this.props.hoa.searchResult[0]) && 
          <button className="addFavButton" onClick={ () => this.props.addFavInfo(this.props.user.user.id, this.props.hoa.searchResult[0].id)}>Add to Favorites</button>}
